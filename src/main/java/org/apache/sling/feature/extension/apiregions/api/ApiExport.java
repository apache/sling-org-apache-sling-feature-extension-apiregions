...
public class ApiExport implements Comparable<ApiExport> {
        ...
        if (dValue.getValueType() == ValueType.STRING) {
            ...
            if (depObj.containsKey(MSG_KEY)) {
                ...
                if (val.getValueType() != ValueType.OBJECT) {
                    throw new IOException("Export " + this.getName() + " has wrong type for " + MEMBERS_KEY + " : "
                            + val.getValueType().name());
                }
                for (final Map.Entry<String, JsonValue> memberProp :
                        val.asJsonObject().entrySet()) {
                    if (memberProp.getValue().getValueType() == ValueType.STRING) {
                        final DeprecationInfo info =
                                new DeprecationInfo(((JsonString) memberProp.getValue()).getString());
                        ...
                        if (!memberObj.containsKey(MSG_KEY)) {
                            throw new IOException("Export " + this.getName() + " has wrong type for member in "
                                    + MEMBERS_KEY + " : "
                                    + memberProp.getValue().getValueType().name());
                        }
                        ...
                        throw new IOException(
                                "Export " + this.getName() + " has wrong type for member in " + MEMBERS_KEY + " : "
                                        + memberProp.getValue().getValueType().name());
                    }
                }
            }
        } else {
            throw new IOException("Export " + this.getName() + " has wrong type for " + DEPRECATED_KEY + " : "
                    + dValue.getValueType().name());
        }
    }
    ...
    JsonValue deprecationToJSON() {
        ...
        if (dep.getPackageInfo() != null) {
            if (dep.getPackageInfo().getSince() == null
                    && dep.getPackageInfo().getForRemoval() == null
                    && dep.getPackageInfo().getMode() == null) {
            ...
            for (final Map.Entry<String, DeprecationInfo> memberEntry :
                    dep.getMemberInfos().entrySet()) {
                if (memberEntry.getValue().getSince() == null
                        && memberEntry.getValue().getForRemoval() == null
                        && memberEntry.getValue().getMode() == null) {
                    membersBuilder.add(
                            memberEntry.getKey(), memberEntry.getValue().getMessage());
                } else {
    ...
    JsonValue toJSONValue() {
        ...
        if (this.getPreviousArtifactId() != null) {
            expBuilder.add(
                    PREVIOUS_ARTIFACT_ID_KEY, this.getPreviousArtifactId().toMvnId());
        }
        ...
        if (val.getValueType() == ValueType.STRING) {
            ...
            if (!name.startsWith("#")) {
                ...
                if (!region.add(export)) {
                    throw new IOException(
                            "Export " + export.getName() + " is defined twice in region " + region.getName());
                }
                return export;
            }
            return null;
                ...
                if (NAME_KEY.equals(key)) {
                    ...
                    if (setPreviousArtifact) {
                        throw new IOException("Export " + export.getName()
                                + " is defining previous artifact id twice in region " + region.getName());
                    }
            ...
            throw new IOException("Region " + region.getName() + " has wrong type for package export : "
                    + val.getValueType().name());
        }
    }
    ...
    public boolean equals(Object obj) {
        ...
        return Objects.equals(deprecation, other.deprecation)
